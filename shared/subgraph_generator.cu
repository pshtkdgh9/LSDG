...
template <class E>
void SubgraphGenerator<E>::generate(Graph<E> &graph, Subgraph<E> &subgraph)//,cudaStream_t stream0
{
	//std::chrono::time_point<std::chrono::system_clock> startDynG, finishDynG;
	//startDynG = std::chrono::system_clock::now();
	
	prePrefix<<<graph.num_nodes/512+1, 512>>>(d_activeNodesLabeling, d_activeNodesDegree, graph.d_outDegree, graph.d_label1, graph.d_label2, graph.num_nodes);//activenode只生成一次
	//prePrefix<<<graph.num_nodes/512+1, 512,0,stream1>>>(d_activeNodesLabeling, d_activeNodesDegree, graph.d_outDegree, graph.d_label11, graph.d_label22, graph.num_nodes);

	cudaDeviceSynchronize();
	// cudaMemcpy(graph.d_label11, graph.label1, sizeof(bool), cudaMemcpyDeviceToDevice);
	// cudaMemcpy(graph.d_label22, graph.label1, sizeof(bool), cudaMemcpyDeviceToDevice);

	// graph.d_label11=graph.d_label1;//graph.d_label的赋值
	// graph.d_label22=graph.d_label2;
	//prePrefix<<<graph.num_nodes/512+1, 512,0,stream1>>>(d_activeNodesLabeling, d_activeNodesDegree, graph.d_outDegree, graph.d_label1, graph.d_label2, graph.num_nodes);
	...
	subgraph.numActiveNodes = thrust::reduce(ptr_labeling, ptr_labeling + graph.num_nodes);//初始化numactivenodes
