...
int main(int argc, char** argv)
{
	...
	*/
	...
	while (subgraph.numActiveNodes>0)    //整个子图
	{
		...
		for(int i=0; i<partitioner.numPartitions; i++) //<
		{
			// DynamicDetect(dynamic_number,partitioner.numPartitions);

			// uint DynamicDetect(int dynamic_number, uint numPartitions)
			// {
			// 	if(numPartitions<24)
			// 	return dynamic_number;
			// 	if(numPartitions>=24)
			// 	{
			// 		dynamic_number=numPartitions/24;
			// 		return dynamic_number;
			// 	}

			// 	//int k=ceil(dynamic_number);
			// }	
			cudaDeviceSynchronize();
		...
		if(i==1||i==2||i==3)
		{
			...
			do
			{
				...
				cudaDeviceSynchronize();
					...
					cc_async<<<partitioner.partitionNodeSize[i]/256 + 1,256,0,stream2>>>(partitioner.partitionNodeSize[i],//host   partitioner.partitionNodeSize[i]/512 + 1 , 512
						...
						(itr%2==1) ? graph.d_label222 : graph.d_label111);		//cudamemcpycudaDeviceSynchronize();
		...
		else
		{
			...
			do
			{
				...
				cudaDeviceSynchronize();
					...
					cc_async<<<partitioner.partitionNodeSize[i]/256 + 1,256,0,stream0>>>(partitioner.partitionNodeSize[i],//host   partitioner.partitionNodeSize[i]/512 + 1 , 512
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					cc_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream1>>>(partitioner.partitionNodeSize[i],//host
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					cc_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream2>>>(partitioner.partitionNodeSize[i],//host
							partitioner.fromNode[i],//host
							partitioner.fromEdge[i],//host
							subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
							subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					cc_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream3>>>(partitioner.partitionNodeSize[i],//host
								partitioner.fromNode[i],//host
								partitioner.fromEdge[i],//host
								subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
								subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					cc_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream4>>>(partitioner.partitionNodeSize[i],//host
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					cc_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream5>>>(partitioner.partitionNodeSize[i],//host
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					cc_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream6>>>(partitioner.partitionNodeSize[i],//host
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					cc_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream7>>>(partitioner.partitionNodeSize[i],//host
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
