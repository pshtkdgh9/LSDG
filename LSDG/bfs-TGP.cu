...
int main(int argc, char** argv)
{
	...
	*/
	...
	//printf("1111\n");
	Subgraph<OutEdge> subgraph(graph.num_nodes, graph.num_edges);//duixiang
	
	SubgraphGenerator<OutEdge> subgen(graph); //duixinag
	//printf("*****************\n");
	...
	while (subgraph.numActiveNodes>0)    //整个子图
	{
		...
		// a super iteration
		cout<<partitioner.partitionNodeSize[0]<<"*****"<<partitioner.partitionEdgeSize[0]<<endl;
		cout<<partitioner.partitionNodeSize[1]<<"*****"<<partitioner.partitionEdgeSize[1]<<endl;
		cout<<partitioner.partitionNodeSize[2]<<"*****"<<partitioner.partitionEdgeSize[2]<<endl;
		cout<<partitioner.partitionNodeSize[3]<<"*****"<<partitioner.partitionEdgeSize[3]<<endl;
		...
		//{			
			cudaDeviceSynchronize();
			gpuErrorcheck(cudaMemcpyAsync(subgraph.d_activeEdgeList, subgraph.activeEdgeList + partitioner.fromEdge[i], (partitioner.partitionEdgeSize[i]) * sizeof(OutEdge), cudaMemcpyHostToDevice,stream0));
			...
			do
			{
				...
				cudaDeviceSynchronize();
					...
					bfs_async<<<partitioner.partitionNodeSize[i]/256 + 1,256,0,stream1>>>(partitioner.partitionNodeSize[i],//host   partitioner.partitionNodeSize[i]/512 + 1 , 512
			...
			do
			{
				...
				cudaDeviceSynchronize();
					...
					bfs_async<<<partitioner.partitionNodeSize[i]/256 + 1,256,0,stream2>>>(partitioner.partitionNodeSize[i],//host   partitioner.partitionNodeSize[i]/512 + 1 , 512
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
		...
		else
		{
			do
			{
				...
				cudaDeviceSynchronize();
					...
					bfs_async<<<partitioner.partitionNodeSize[i]/256 + 1,256,0,stream0>>>(partitioner.partitionNodeSize[i],//host   partitioner.partitionNodeSize[i]/512 + 1 , 512
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					bfs_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream1>>>(partitioner.partitionNodeSize[i],//host
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					bfs_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream2>>>(partitioner.partitionNodeSize[i],//host
							partitioner.fromNode[i],//host
							partitioner.fromEdge[i],//host
							subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
							subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					bfs_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream3>>>(partitioner.partitionNodeSize[i],//host
								partitioner.fromNode[i],//host
								partitioner.fromEdge[i],//host
								subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
								subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					bfs_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream4>>>(partitioner.partitionNodeSize[i],//host
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					bfs_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream5>>>(partitioner.partitionNodeSize[i],//host
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					bfs_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream6>>>(partitioner.partitionNodeSize[i],//host
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
					...
					bfs_async<<<partitioner.partitionNodeSize[i]/256 + 1, 256,0,stream7>>>(partitioner.partitionNodeSize[i],//host
						partitioner.fromNode[i],//host
						partitioner.fromEdge[i],//host
						subgraph.d_activeNodes,  //  不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
						subgraph.d_activeNodesPointer, //不需要memcpy赋值，只cudamalloc,通过核函数在GPU中赋值
